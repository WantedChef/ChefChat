...
async def test_fails_cat_command_with_missing_file(bash):
    with pytest.raises(ToolError) as err:
        await bash.run(
            BashArgs(command="python -c \"print(open('missing_file.txt').read())\"")
        )
...
async def test_uses_effective_workdir(tmp_path):
    ...
    result = await bash_tool.run(
        BashArgs(command='python -c "import os; print(os.getcwd())"')
    )
...
async def test_handles_timeout(bash):
    with pytest.raises(ToolError) as err:
        await bash.run(
            BashArgs(command='python -c "import time; time.sleep(2)"', timeout=1)
        )
...
async def test_truncates_output_to_max_bytes(bash):
    ...
    result = await bash_tool.run(
        BashArgs(command="python -c \"print('abcdefghij', end='')\"")
    )
